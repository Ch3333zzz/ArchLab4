...
# Организация памяти
    Адресация: word-индексы (word = 4 байта). 0 — низкие адреса, mem_cells-1 — высокие.
...
##    Хранение строк
    в виде pstr (Length-prefixed (Pascal string))
...
## Какие виды памяти и регистров доступны программисту?
    ACC (основной регистр для вычислений), PC (косвенно), доступ к памяти с помощью команд LOAD_MEM, STORE_MEM, LOAD_IND, STORE_IND, AGET/ASET и т.д;MMIO-порты: два специальных адреса (по умолчанию 0xFFF0 — mmio_in, 0xFFF1 — mmio_out). Они отображены в память и доступны обычными инструкциями LOAD_MEM/STORE_MEM.Стековые области в памяти: runtime stack (для локалей/временных), frame-stack (сохранённые FP), call-stack (адреса возврата), interrupt-stack (сохранённый контекст прерывания) — все доступны только через реализацию инструкций/сервисы (mem_push/mem_pop, CALL/RET, IRET и т.д.). Программист оперирует ими косвенно (через инструкции языка).
...
### Статические данные (константы, строковые литералы, глобальные переменные) хранятся в DATA в начале памяти:
    ...
    В начале DATA выделяется string_pool_cells слов для пулa строк (pstr). Компилятор сначала пытается размещать все pstr туда.

    Далее компилятор добавляет глобальные переменные (по одному слову на переменную).
...
Статические строки (pstr) хранятся как «length-prefixed Pascal-style»: в первой ячейке (word) записывается длина len, затем len слов — по одному символу (код символа в младшем байте слова). Т.е. строка занимает 1 + len машинных слов.

Размещение:

Компилятор пытается поместить все pstr в зарезервированный пул (первые string_pool_cells слов). Строки размещаются последовательно: сначала header (len) в следующем свободном слове пула, затем символы — подряд.

Если пул переполнен, компилятор размещает pstr в конце секции данных, также последовательно.
## Работа с литералами
### Численные
    Численные литералы (целые) компилятор обычно кодирует как LOAD_IMM <imm> — т.е. immediate аргумент инструкции, аргумент инструкции — 32-битное значение, поэтому литерал помещается целиком (если он в диапазоне 32-бит). При декоде аргумент трактуется как signed 32-bit.
### Строковые литералы
    сохраняется в string-pool, в конец секции данных
# Система команд
    ...
    Машинное слово: 32 бита (signed при чтении через read_word/write_word и при декоде immediates).
## Типы данных и представление

    Машинное слово: 32-бит signed two's-complement.

    Числа: компилятор обычно генерирует LOAD_IMM <imm> для литералов — imm укладывается в 32 бита.
...
## Адресация и представление инструкций
    ...
    word64 = (opcode_byte << 32) | (arg & 0xFFFFFFFF)
    bytes = struct.pack("<Q", word64)
    => порядок байтов: [arg_byte0, arg_byte1, arg_byte2, arg_byte3, opcode_byte, 0x00, 0x00, 0x00].
    ...
    Патчинг меток (компилятор): компилятор emit сначала записывает encode_instr(op, 0) и запоминает позицию; позже _write_arg_at(pos, addr) перезаписывает первые 4 байта инструкции (аргумент) оставляя opcode неизменным.
## Режимы адресации
    ...
    Indirect via stack — команды LOAD_IND/STORE_IND берут адрес со стека (mem_pop()), затем читают/пишут по этому адресу.
...
## Ввод-вывод и прерывания
    ...
    STORE_MEM(mmio_out, v) — добавляет v & 0xFF в mmio_out_buffer и output_buffer, обновляет pstr в памяти по адресу mmio_out.
...
### Прерывания:
    ...
    Ввод моделируется как событие (по расписанию). При событии символа VM вызывает mem_write_word(mmio_in, ord(ch)) и помечает last_interrupt_source="IN". Затем CU вызывает handle_interrupt() (если interruption_allowed).
    ...
    Обработчик прерывания — обычный код/процедура, написанный на вашем Lisp (компилируется в CODE); возврат осуществляется IRET (или RET + ISP logic) — при IRET/восстановлении читаются значения обратно и in_interruption=False.
...
### Таблица команд
```
| Opcode (такты)| Имя         | Семантика                                                                                       | Примечание / детали                 |
|---------------|-------------|-------------------------------------------------------------------------------------------------|-------------------------------------|
...
```
# Транслятор
    Имя утилиты: parser.py (в репозитории — модуль parser)
...
## Принципы работы транслятора — этапы
    ...
    Чтение исходника
    Файл считывается как UTF-8 байтовая строка.

    Токенизация (tokenize)
    Регулярным выражением выделяются токены: ( ) числа, идентификаторы, строки в двойных кавычках, комментарии (; …) игнорируются. Результат — плоский список токенов.
    ...
    Запись файлов
    После удачной компиляции байты code и data записываются в указанные файлы.
# Модель процессора
    Имя утилиты: processor.py
...
# Тестирование
    ./golden/add64.yaml - сложение 64битных чисел
    ./golden/cat.yaml - печатать данные, поданные через ввод.
    ./golden/difference_100.yaml - реализация алгоритма по варианту (разность суммы квадратов и квадрата суммы первых ста чисел)
    ./golden/fib.yaml - проверка работы рекурсивной функции (числа Фибоначчи)
    ./golden/hello_user_name.yaml - запросить у пользователя его имя, считать его, вывести на экран приветствие
    ./golden/hello_world.yaml - вывести "Hello, World!"
    ./golden/pause_test.yaml - проверка возможности остановки процессора на любом тике
    ./golden/sort.yaml - пользователь загружает в систему список чисел и программа выводит их в отсортированном формате;
    ./golden/statement.yaml - проверка некоторых выражений
