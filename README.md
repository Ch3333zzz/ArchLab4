...
|  - main (основная код) затем функции (defun)                   |
+----------------------------------------------------------------+
...
+----------------------------------------------------------------+
| INTERRUPT-STACK (ISP area, растёт вниз)                        |
|  - хранит сохранённый контекст при входе в прерывание          |
+----------------------------------------------------------------+
| FRAME-STACK (FSP area, растёт вниз)                            |
|  - хранит сохранённые старые значения FP (frame_push/pop)      |
+----------------------------------------------------------------+
| CALL-STACK (CP area, растёт вниз)                              |
|  - хранит return addresses                                     |
+----------------------------------------------------------------+
...
## Архитектура
Архитектура Фон-Неймана (память данных и команд объединены)
...
- ACC — аккумулятор. Основной регистр, в котором компилятор оставляет результаты выражений;

- PC — указывает на следующую выполняемую инструкцию.

- SP — указатель следующей свободной ячейки runtime-стека.

- FP — указывает базовую позицию для текущего фрейма.

- FSP — указатель для вершины frame-stack.

- CP — указатель для вершины call-stack.

- SP —  адрес следующей свободной ячейки runtime stack.
